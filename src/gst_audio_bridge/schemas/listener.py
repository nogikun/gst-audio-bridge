...
class ListenerConfig(BaseModel):
    ...
    data_format: Literal["raw", "encoded", "torch_audio"] = Field(
        "raw",
        description="Format of the audio data: 'raw' (bytes), 'encoded' (opus), 'torch_audio' (torch.Tensor).",
    )
    chunk_duration_ms: int = Field(
        20,
        description="Duration of each audio chunk in milliseconds. Common values: 10, 20, 40, 60ms.",
    )
...
class ListenerInitArgs(BaseModel):
    ...
    listen_ip: str = Field(
        "0.0.0.0", description="IP address to listen on for incoming streams."
    )
    audio_port: int = Field(..., description="UDP port for audio streaming.")
    video_port: Optional[int] = Field(
        None, description="UDP port for video streaming (optional)."
    )
